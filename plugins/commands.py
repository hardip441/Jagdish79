...
if data.split("-", 1)[0] == "VJ":
    ...
    # Run referral logic inside the async handler
    vj = await referal_add_user(user_id, message.from_user.id)

    if vj and PREMIUM_AND_REFERAL_MODE:
        await message.reply(
            f"You have joined using the referral link of user with ID {user_id}\n\n"
    ...
    await client.send_message(
        chat_id=user_id,
        text=(
            f"{message.from_user.mention} started the bot with your referral link\n\n"
            f"Total Referrals - {num_referrals}"
        )
    )
    ...
    if num_referrals == REFERAL_COUNT:
        ...
        if seconds > 0:
            ...
            await client.send_message(
                chat_id=user_id,
                text=(
                    f"You Have Successfully Completed Total Referral.\n\n"
                    f"You Were Added To Premium For {REFERAL_PREMEIUM_TIME}"
                )
            )
    return
...
if not files_:
    ...
    try:
        if not await db.has_premium_access(message.from_user.id):
            if VERIFY == True and not await check_verification(client, message.from_user.id):
                btn = [[
                    InlineKeyboardButton(
                        "ᴠᴇʀɪғʏ",
                        url=await get_token(
                ...
                text = (
                    ...
                    "ᴘʟᴇᴀꜱᴇ ᴄʟɪᴄᴋ ᴏɴ ᴠᴇʀɪғʏ & "
                    "ɢᴇᴛ ᴜɴʟɪᴍɪᴛᴇᴅ ᴀᴄᴄᴇꜱꜱ ғᴏʀ ᴛᴏᴅᴀʏ</b>"
                )

                if PREMIUM_AND_REFERAL_MODE == True:
                    text += (
                        "<b>\n\nɪғ ʏᴏᴜ ᴡᴀɴᴛ ᴅɪʀᴇᴄᴛ ғɪʟᴇꜱ "
                        "ᴡɪᴛʜᴏᴜᴛ ᴀɴʏ ᴠᴇʀɪғɪᴄᴀᴛɪᴏɴꜱ "
                        "ᴛʜᴇɴ ʙᴜʏ ʙᴏᴛ ꜱᴜʙꜱᴄʀɪᴘᴛɪᴏɴ ☺️"
                        "\n\n💶 ꜱᴇɴᴅ /plan ᴛᴏ ʙᴜʏ ꜱᴜʙꜱᴄʀɪᴘᴛɪᴏɴ</b>"
                    )
        ...
        k = await msg.reply(
            text="<blockquote><b><u>❗️❗️❗️IMPORTANT❗️❗️❗️</u></b>\n\n"
                 "ᴛʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴅᴇʟᴇᴛᴇᴅ "
                 "ɪɴ <b><u>10 mins</u></b> 🫥\n\n"
                 "<b><i>ᴘʟᴇᴀsᴇ ғᴏʀᴡᴀʀᴅ ᴛʜɪs ᴍᴇssᴀɢᴇ "
                 "ᴛᴏ ʏᴏᴜʀ sᴀᴠᴇᴅ ᴍᴇssᴀɢᴇs</i></b></blockquote>"
        )
